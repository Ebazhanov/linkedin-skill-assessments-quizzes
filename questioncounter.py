    ...
    for entry in entries:
        if entry.is_dir() and not entry.name.startswith('.') and not entry.name.startswith('assets') and not \
            ...
            with open(filename) as f:
                text = f.read()
                answered = text.count("[x]")
                unanswered = (text.count("[ ]") - 3 * answered)/4
                total = answered + unanswered
                print(f"{filename} {total} [x] {answered} [ ] {unanswered}")

#
# listers = ['./accounting/accounting-quiz.md', './adobe-acrobat/adobe-acrobat-quiz.md',
#            './adobe-illustrator/adobe-illustrator-quiz.md','./adobe-in-design/adobe-in-design-quiz.md']
# for thing in listers:
...
# ./aws/aws-quiz.md 98.5 [x] 99 [ ] -0.5
# ./aws-lambda/aws-lambda-quiz.md 51.0 [x] 49 [ ] 2.0
