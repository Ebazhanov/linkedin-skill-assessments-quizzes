...
#### Q2. Which of the following is a reason why using this line is considered a bad practice?
```
Using namespce std;
```
 - [ ] The compiled code is always bigger because of all of the imported symbols.
 - [x] If the code uses the function defined in two different libraries with the same prototype but possibly with different implementation , there will be a compilation error due to ambuiguity.
...
#### Q11. Suppose you need to keep a data struct with permission to access some resource base on the days of week, but you can't use a bool variable for each day. You need to use one bit per day of the week. Which of the following is a correct implementation of a structure with bit fields for this application?
- [x] 
```
typedef struct {
    int sunday:1;
...
auto x = 4000.22;
```
 - [x] It specifies that the type of x will be deducted from the initializer - in this case, double.
...
#### Q.17 What is the meaning of the two parts specified between parentheses in a range-based for loop, separated by a colon?
...
#### Q.19 What results from executing this code snippet?
```
int x=5, y=2;
if(x & y) { 
    /*_part A_*/ 
...
#### Q.20 What is a valid definition for the `get_length` function, which returns the length of a null-terminated string?
- [x] 
```
